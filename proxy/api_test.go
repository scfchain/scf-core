...
func TestEthCall(t *testing.T) {
	...
	backend, err := NewBackend(&Config{ArchiveUrl: "https://api-archived.fchainchain.com/rpc", RpcUrl: "https://api-archived.fchainchain.com/rpc"}, &ethConfig)
	if err != nil {
		t.Fatal(err)
	}
	api := newAPI(backend)
...
func TestEthBlockNumber(t *testing.T) {
	ethConfig := ethconfig.Defaults
	ethConfig.RPCEVMTimeout = time.Second * 50
	backend, err := NewBackend(&Config{ArchiveUrl: "https://api-archived.fchainchain.com/rpc", RpcUrl: "http://34.121.216.144:8545"}, &ethConfig)
	if err != nil {
		t.Fatal(err)
	}
