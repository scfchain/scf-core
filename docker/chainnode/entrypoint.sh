...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    2020 )
      ...
      params="$params $FCHAIN_PARAMS"
      ;;
    2021 )
      genesisPath="testnet.json"
      params="$params --gcmode archive --http.api eth,net,web3,debug,consortium $FCHAIN_PARAMS"
      ;;
    2022 )
...
# data dir
if [[ ! -d $datadir/fchain ]]; then
  echo "No blockchain data, creating genesis block."
  fchain init $genesisPath --datadir $datadir 2> /dev/null
...
accountsCount=$(
  fchain account list --datadir $datadir  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    fchain account import ./private_key \
      --datadir $datadir \
